...
def print_if_size_appropriate(dir):
    if int(dir.get_size()) > spacetofind:
...
with open('input.txt') as input:
    ...
    sparespace = maxsize - top_level_dir.get_size()
    spacetofind = requiredspace - sparespace
    ...
    print(spacetofind)
    lowestdir = maxsize
    for di in dirs_to_add:
        if di.get_size() < lowestdir:
            lowestdir = di.get_size()
    print(lowestdir)